...
public class OneMaxRunner {
	public static void run(int runNumb) throws FileNotFoundException 
	{
		...
		for (OneMaxFitnessType action : OneMaxFitnessType.values()) {
		...
		RLFitnessEvaluator fitnessEvaluator = new OneMaxFitnessEvaluator(OneMaxFitnessType.ONEMAX.toString());
		...
		PrintWriter pwEA = new PrintWriter("ONEMAX_" + populationSize + "_" + eliteCount + "_" + "target_value" + "_MEAN_MS1_" + runNumb);
